...
clBrown = [0.6,0.2,0]; clColor1 = [0.8,0.2,0.9]; clColor2 = [0.8,0.9,0.1];
aColor = [clBlue; clMen; clGreen;  clRed;  clBrown; clBlack; clColor1; clColor2];
aMarker = ['*:', 'o:', 'd:', '^:', 's:', 'v:','h:', 'p:'];
markerSize = 7;
lnWidthSim0 = 1.6;
lnWidthSd0 = 0.5;
...
% draw the zhbar 
text(1.0,0.2,['(\phi_i=',num2str(varphi,'%1.3f)')],  'fontsize', fontSize, 'fontname','arial' )
...
['C6:|\sigma_i|=', num2str(asigd(6), '%1.2f')], ...
['C7:|\sigma_i|=', '\infty'  ]);
...
% ['C1:|\sigma_S|=', num2str(asigs(1), '%1.2f'),  ', |\sigma_D|=',  num2str(asigd(1), '%1.2f'), ',\vartheta=',   num2str(ahbar(1))],...
% ['C2:|\sigma_S|=', num2str(asigs(2), '%1.2f'),  ', |\sigma_D|=',  num2str(asigd(2), '%1.2f'), ', \vartheta=',  num2str(ahbar(2))],...
% ['C3:|\sigma_S|=', num2str(asigs(3), '%1.2f'), ', |\sigma_D|=',   num2str(asigd(3), '%1.2f'), ', \vartheta=',  num2str(ahbar(3))], ...
% ['C4:|\sigma_S|=', num2str(asigs(4), '%1.2f'),  ', |\sigma_D|=',  num2str(asigd(4), '%1.2f'), ', \vartheta=',  num2str(ahbar(4))], ...
% ['C5:|\sigma_S|=', num2str(asigs(5), '%1.2f'),   ', |\sigma_D|=', num2str(asigd(5), '%1.2f'), ', \vartheta=',  num2str(ahbar(5))]);
% ['|\sigma|=', num2str(asigm(6), '%1.2f'), ', \vartheta=',  num2str(hbar)]);
ylabel('CDF/Pr[J_{b,i}<z]')
xlabel('z')
grid on;
